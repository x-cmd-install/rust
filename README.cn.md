# rust

[English version](./README.md)

Empowering everyone to build reliable and efficient software.

![rust](https://repo.x-cmd.io/rust.svg?lang=zh)

## 安装

```sh
x install rust
```

## 代码洞察

合计: **3,918,186** 行代码（覆盖前 5 种语言、共 **38995** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| Rust | 2,941,290 | 331,178 | 404,304 | 38777 |
| Json | 354,909 | 0 | 2 | 57 |
| Html | 315,546 | 19 | 250 | 139 |
| Xml | 158,446 | 18 | 73 | 9 |
| Yaml | 37,104 | 427 | 2,320 | 13 |

## OpenSSF Scorecard 评分

总评分: **7.1 / 10**

评分最低的几项:

- **Packaging** (-1/10) — packaging workflow not detected
- **Token-Permissions** (0/10) — detected GitHub workflow tokens with excessive permissions
- **CII-Best-Practices** (0/10) — no effort to earn an OpenSSF best practices badge detected

## 源代码

- **上游仓库**: <https://github.com/rust-lang/rust>
- **官网**: <https://www.rust-lang.org>
- **许可证**: Apache-2.0

## 发布

- **最新版本**: `1.98.1` (2026-09-03)
- **最近提交**: 2026-09-11

## 流行度

- **Star**: 118,397 · **Fork**: 15,554 · **开放 issue**: 63,846 · **贡献者**: 6,847

## 累计统计

- **发布数**: 155 · **已合并 PR**: 74277 · **开放 PR**: 1387 · **已关闭 issue**: 52623 · **开放 issue**: 11223 · **提交数**: 339815

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-12 | 2 | 680 | 353 | 113 | 250 | 2534 |
| last60d | 2026-07-13 | 3 | 1537 | 524 | 322 | 480 | 6066 |
| 90d | 2026-06-13 | 5 | 2233 | 633 | 488 | 634 | 8968 |
| last180d | 2026-03-15 | 8 | 4235 | 838 | 1028 | 1094 | 17517 |
| 360d | 2025-09-16 | 15 | 7811 | 1098 | 2306 | 2040 | 31810 |
| last720d | 2024-09-21 | 25 | 16209 | 1300 | 5575 | 3827 | 66152 |

## 发行版状态

在 [repology.org](https://repology.org/project/rust) 上共有 **1099** 个发行版报告此项目。**109** 个 ✅ 已是最新上游版本，**363** 个 ⚠️ 使用旧版本。

| 发行版 | 版本 | 状态 |
|--------|------|------|
| Debian unstable | `1.85.0` | 🪦 legacy |
| Debian 14 | `1.90.0` | 🪦 legacy |
| Debian 13 | `1.85.0` | ⚠️ outdated |
| Ubuntu 26.04 LTS | `1.91.1` | 🪦 legacy |
| Ubuntu 24.04 LTS | `1.82.0+dfsg0ubuntu0` | 🪦 legacy |
| Arch | `1.98.1` | ✅ latest |
| Homebrew | `1.98.1` | ✅ latest |
| Fedora rawhide | `1.98.1` | ✅ latest |
| Nix unstable | `1.97.1` | ⚠️ outdated |
| Void | `1.98.0` | ⚠️ outdated |
| Alpine edge | `1.97.0` | ⚠️ outdated |
| openSUSE Tumbleweed | `1.98.0` | ⚠️ outdated |

## 改进这些数据

rust 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `rust` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/rust.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260911.yml` · 2026-09-11T20:54:41Z._
